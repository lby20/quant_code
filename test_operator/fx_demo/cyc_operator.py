...
def div_cs(a, b): return 100 * np.where(a + b == 0, 0, (a - b) / (a + b))
def div_cs_abs(a, b): return 100 * np.where(a + b == 0, 0, (a - b) / (a + b))
def div(a, b): return np.where(b == 0, 0, a / b)
...
def growth(a,b): return 100 * np.where(b == 0, 0, a / b - 1)
        ...
        for name in all_attr:
            if isinstance(getattr(self, name), np.ndarray):
                ...
                if nd == 2:
                    ...
                    new_attr[np.isinf(new_attr) | np.isnan(new_attr)] = 0
...
class ts_sum_div:
    ...
    def __call__(self, x, y):
        ...
        self._info[:] = np.where(self.y_sum == 0, 0, self.x_sum / self.y_sum)
...
class ts_sum_div_cs:
    ...
    def __call__(self, x, y):
        ...
        self._info = 100 * np.where(np.isinf(self._info),0,self._info)
...
class ts_sum_div_cs_abs:
    ...
    def __call__(self, x, y):
        ...
        self._info = np.fabs(100 * np.where(np.isinf(self._info),0,self._info))
...
class ts_tcv:
    ...
    def __call__(self, data):
        ...
        # self._info[np.isinf(self._info)] = np.nan
        self._info[np.isinf(self._info)] = 0
        self.new_idx += 1
        if self.new_idx > self.window - 1:
            self.new_idx = 0
        return self._info
